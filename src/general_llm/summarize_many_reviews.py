...
def pase_review(review: str) -> tuple[str, str, str]:
    ...
    return (
        review[advantages_pos:disadvantages_pos], # .strip('Достоинства:')
        review[disadvantages_pos:comments_pos], # .strip('Недостатки:')
        review[comments_pos:],  # .strip('Комментарий:')
    )
...
#     advantages: list = Field(..., description="list of main advantages", required=True)
...
chat_model = LlamaCpp(
    ...
    temperature=0.0,
    # repeat_penalty=1.5,
    grammar_path='/grammars/advantages_disadvantages.gbnf',
    stop=["<|eot_id|>", ],
)
...
if __name__ == '__main__':
    ...
    i = 0
    for item_name, reviews_body in output_dict.items():
        ...
        for ua in united_advantages_list:
            ...
            #     logger.critical(f"{item_name}--advantages: {e}")
        for ud in united_disadvantages_list:
            ...
            # except Exception as e:
            #     logger.critical(f"{item_name}--disadvantages: {e}")
        ...
        # con.write_one({item_name: summarized_json})
        summarized_reviews.append(summarized_json)



    logger.info(summarized_reviews[:5])
